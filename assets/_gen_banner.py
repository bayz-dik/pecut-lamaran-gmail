...
def main():
  ...
  <style>
    ...
    .l rect {{ fill: {INK}; }}
    .l1 {{ animation: sweep 2.8s ease-in-out infinite 0s; }}
    .l2 {{ animation: sweep 2.8s ease-in-out infinite .18s; }}
    .l3 {{ animation: sweep 2.8s ease-in-out infinite .36s; }}
    .l4 {{ animation: sweep 2.8s ease-in-out infinite .54s; }}
    .l5 {{ animation: sweep 2.8s ease-in-out infinite .72s; }}
    @keyframes sweep {{
      0%, 100% {{ fill: {INK}; }}
      38%      {{ fill: {TEAL}; }}
      62%      {{ fill: {TEAL}; }}
    ...
    @media (prefers-reduced-motion: reduce) {{
      .l1, .l2, .l3, .l4, .l5, .env, .dot {{ animation: none; }}
    }}
  </style>
